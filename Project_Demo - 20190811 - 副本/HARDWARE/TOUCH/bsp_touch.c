...
uint8_t TP_Adjust(void)
{
	...
	LCD_Clear(WHITE);	// 清屏
	POINT_COLOR = BLACK;
	LCD_ShowString(40, 40, 200, 100, 16, (uint8_t*)TP_REMIND_MSG_TBL); // 显示提示信息
...
uint8_t TP_Init(void)
{
	...
	{
		FT6236_Init();
		tp_dev.scan = FT6236_Scan;		       // 扫描函数指向FT623触摸屏扫描
	...
	else
	{
	    ...
	    // PC0---->T_PEN
		// PC1---->T_MISO
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1; // PC0/PC1 设置为上拉输入
	    ...
	    // PC2---->T_MOSI		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;              // PC2设置为推挽输出
	    ...
	    // PC3---->T_CS
		// PC4---->T_SCK
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_4; // PC3、PC4设置为推挽输出
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;          // 输出模式
		GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;         // 推挽输出
		GPIO_Init(GPIOC, &GPIO_InitStructure);                 // 初始化
		...
		{
			LCD_Clear(WHITE);	// 清屏
			LCD_ShowString(10, 30, 280, 24, 24, "Adjustment Fail！");
			return 0;
		}
		LCD_Clear(WHITE);		// 清屏
	}
