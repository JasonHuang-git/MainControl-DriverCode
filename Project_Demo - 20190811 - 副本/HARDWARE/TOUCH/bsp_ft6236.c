...
uint8_t FT6236_Init(void)
{
    ...
    // 读取版本号
    FT6236_RD_Reg(FT_ID_G_LIB_VERSION,&temp[0],2);  
    if(temp[0]==0x10&&temp[1]==0x05)// 版本:0X1005
    {
		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			for(i=0;i<5;i++)
			{
				if(tp_dev.sta&(1<<i))	// 触摸有效?
				{
					...
					{
#if (DFT_SCAN_DIR==L2R_U2D)
						// 从上往下，从右往左
						tp_dev.y[i]=((uint16_t)(buf[0]&0X0F)<<8)+buf[1];
						tp_dev.x[i]=-(((uint16_t)(buf[2]&0X0F)<<8)+buf[3]-scan_dir.xs);
#elif (DFT_SCAN_DIR==R2L_D2U)
						tp_dev.x[i]=((uint16_t)(buf[2]&0X0F)<<8)+buf[3];
						tp_dev.y[i]=-(((uint16_t)(buf[0]&0X0F)<<8)+buf[1]-scan_dir.ys);
#endif
					}else
					{
#if (DFT_SCAN_DIR==L2R_U2D)
						// 从上往下，从左往右
						tp_dev.x[i]=-((((uint16_t)(buf[0]&0X0F)<<8)+buf[1])-scan_dir.ys);
						tp_dev.y[i]=-(((uint16_t)(buf[2]&0X0F)<<8)+buf[3]-scan_dir.xs);
#elif (DFT_SCAN_DIR==R2L_D2U)
