			...
			for(i=0;i<5;i++)
			{
				if(tp_dev.sta&(1<<i))	// 触摸有效?
				{
					...
					if(tp_dev.touchtype&0X01)//横屏
					{
						...
						tp_dev.x[i]=-(((u16)(buf[2]&0X0F)<<8)+buf[3]-scan_dir.xs);
#elif (DFT_SCAN_DIR==R2L_D2U)
						tp_dev.x[i]=((u16)(buf[2]&0X0F)<<8)+buf[3];
						tp_dev.y[i]=-(((u16)(buf[0]&0X0F)<<8)+buf[1]-scan_dir.ys);
#endif
					}else
					{
#if (DFT_SCAN_DIR==L2R_U2D)
						// 从上往下，从左往右
						tp_dev.x[i]=-((((u16)(buf[0]&0X0F)<<8)+buf[1])-scan_dir.ys);
						tp_dev.y[i]=-(((u16)(buf[2]&0X0F)<<8)+buf[3]-scan_dir.xs);
#elif (DFT_SCAN_DIR==R2L_D2U)
